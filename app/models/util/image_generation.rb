...
module ImageGeneration
  def self.generateImage(project_name, day_left, color, current_pct, diff_pct, path)
    ...
    <div style="float:left;">#{project_name}: </div><div style="float:right;">#{day_left} days left. #{diff_percentage} behind schedule.</div>
    <div style="height:20px; margin_bottom;20px; overflow:hidden; background-color:#f5f5f5; border-radius:4px; box-shadow: inset 0 1px 2px rgba(0,0,0,.1); clear:both;">
      ...
      #{diff_pct > 0 ? "<div style=\"width: #{diff_percentage};  color:#{diff_percentage == 0? "#777":"#fff"}; background-color: #dc143c; float: left; height: 100%; min-width: 0%; font-size:12px; line-height:20px; text-align: center; \"></div>":""}
    </div>
    </body>
    </html>
    here
